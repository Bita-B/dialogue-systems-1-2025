...
const grammar: { [index: string]: GrammarEntry } = {
  ...
  // Hard cases testing
  westeros: { person: "Westeros" },
  fereshteh: { person: "Fereshteh" },
  parvaneh: { person: "Parvaneh" },
...
function getPerson(utterance: string) {
  ...
  // matching
  for (const [key, value] of Object.entries(grammar) as [string, GrammarEntry][]) {
    if (value.person) {
  ...
  states: {
    ...
    Greeting: {
      ...
      states: {
        ...
        FinalConfirmation: {
          entry: [
            ({ context }) => console.log(" Speaking final confirmation for:", context.meetingPerson, context.meetingDate, context.fullDay ? "whole day" : context.meetingTime),
            {
              type: "spst.speak",
              params: ({ context }) => ({
                utterance: context.fullDay
                  ? `Your meeting with ${context.meetingPerson} on ${context.meetingDate} for the whole day has been scheduled.`
                  : `Your meeting with ${context.meetingPerson} on ${context.meetingDate} at ${context.meetingTime} has been scheduled.`,
              }),
            },
          ],
...
dmActor.send = function (event: any) {
  if (event.type === "RECOGNISED") {
    const utterance = event.value?.[0]?.utterance || "N/A";
    const confidence: number | undefined = event.value?.[0]?.confidence;
    const allHypotheses = event.value || [];

    console.group("ASR RESULT");
    ...
    if (allHypotheses.length > 1) {
      console.log("All hypotheses:");
      allHypotheses.forEach((h: any, i: number) => {
        console.log(`  [${i}] "${h.utterance}" — confidence: ${h.confidence?.toFixed(4) ?? "N/A"}`);
      });
    }
    console.groupEnd();
  }
  if (event.type === "ASR_NOINPUT") {
    console.log("ASR_NOINPUT: no speech detected");
  }
